...
//
//		
//		GTA:VC Japanese support by lazyuselessman
...
class GameInfo
{
	...
	public:
		GameInfo(Game xgame = GAME_UNK)
			: game(xgame), major(0), minor(0), steam(false), japanese(false), delayed(false)
		{
		...
		// Checks if this is the steam version
		bool IsJapanese()		{ return japanese; }
	...
	private:
		...
		bool DetectVC()
		{
			if(ReadMemory<uint32_t>(0x667BF0, true) == 0x53E58955)
				game = VC, major = 1, minor = 0, region = REGION_UNK, steam = false, japanese = false;
			else if(ReadMemory<uint32_t>(0x667C40, true) == 0x53E58955)
				game = VC, major = 1, minor = 1, region = REGION_UNK, steam = false, japanese = false;
			else if(ReadMemory<uint32_t>(0xA402ED, true) == 0x56525153)
				game = VC, major = 1, minor = 1, region = REGION_UNK, steam = true, japanese = false;		// You have to check if steam
			else if(ReadMemory<uint32_t>(0xACD0A2, true) == 0x6AEC8B55)
				game = VC, major = 1, minor = 0, region = REGION_UNK, steam = false, japanese = true;
			else	
				return false;
